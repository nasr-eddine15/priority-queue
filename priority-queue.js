...
// USING STRICT MODE ALLOWS FOR MORE RELIABILITY
...
function PriorityQueue(size) {
  ...
  this.add = function (value, priority) {
    // CHANGE TO ===
    // IF STATEMENT LOGIC WAS NOT INSIDE {}
  ...
  this.Pop = function () {
    // maxKey IS NOT INITIALIZED, IT IS JUST USED
...
// USE LET INSTEAD OF VAR IN THIS BLOCK
PriorityQueue.prototype.forEach = function (callback) {
  var keys = Object.keys(this.store).sort();

  for (var a = keys.length; a > 0; a--) {
    for (var b = 0; b < this.store[a].length; b++) callback(this.store[a][b]);
  }
};
